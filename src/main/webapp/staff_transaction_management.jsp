            ...
            <body>
                <main class="main-container">
                    ...
                    <div class="main-content" id="main-content">
                        <form method="get" action="SearchBillServlet" class="w-50 mx-auto mt-3">
                        ...
                        <table class="table table-bordered table-hover w-75 mx-auto">
                            ...
                            <tbody class="table-success" id="billList">
                                ...
                                <c:forEach items="${bills}" var="bill">
                                    <tr <c:if test="${bill.sender.username eq user.username}">class='table-danger'
                                        </c:if>>
                                        <td>${bill.content}</td>
                                        ...
                                        <td>$${bill.price}</td>
                                        <td>
                                            <fmt:formatDate pattern='dd-MMM-yyyy' value='${bill.transactionDate}' />
                                        </td>
                                    </tr>
                                </c:forEach>
                            </tbody>
                                    ...
                                    <c:choose>
                                        <c:when test="${empty search && action == null}">
                                            <li class="page-item ${param.page == index?" active":""}">
                                                <a class="page-link"
                                                    href='StaffBillListServlet?page=${index}'>${index}</a>
                                            </li>
                                        </c:when>
                                        <c:when test="${action != null}">
                                            <li class="page-item ${param.page == index?" active":""}">
                                                <a class="page-link"
                                                    href='StaffBillListServlet?page=${index}&start=${start}&end=${end}'>${index}</a>
                                            </li>
                                        </c:when>
                                        <c:otherwise>
                                            <li class="page-item ${param.page == index?" active":""}">
                                                <a class="page-link"
                                                    href='SearchBillServlet?search=${search}&page=${index}'>${index}</a>
                                            </li>
                                        </c:otherwise>
                                    </c:choose>
                                </c:forEach>
